async function getData()
{
    ...
    let url = "http://api.aviationstack.com/v1/flights?access_key=9cc39ae140aa6ce121de7951a3bab71d&flight_iata=CM444"
    console.log(url);
    fetch(url, {mode: 'cors'})
...
//will be called by button when submit is clicked
getData();
function displayData(data)
{
    ...
    let duration = calculateDuration(departure.slice(0,19), arrival.slice(0,19))
    let gate = data['data']['0']['departure']['gate'];

    let url2 = 'https://test.api.amadeus.com/v1/travel/predictions/flight-delay?originLocationCode=' + depIata + '&destinationLocationCode=' + arrIata + '&departureDate=' + depDate + '&departureTime=' + depTime.slice(0,2) + '%3A' + depTime.slice(3,5) + '%3A00&arrivalDate=' + arrDate + '&arrivalTime' + arrTime.slice(0,2) + '%3A' + arrTime.slice(3,5) + '%3A00aircraftCode='+ aircraftCode + '&carrierCode=' + carrierCode + '&flightNumber='  + flightNumber + '&duration=' + duration;
    console.log(url2);
